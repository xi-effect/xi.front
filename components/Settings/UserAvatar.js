    ...
    >
        ...
        >
            {items.map((item, index) => (<Grid
                ...
                sx={{ maxWidth: 240, m: 1, }}
            >
                <IconButton onClick={() => userSt.setSettingsSecond("avatar", item.type, userSt.settings.avatar[item.type] === 0 ? item.length - 1 : userSt.settings.avatar[item.type] - 1)}>
                    <NavigateBeforeIcon />
                </IconButton>
                <Typography>
                    {`${item.label} № ${userSt.settings.avatar[item.type] + 1}`}
                </Typography>
                <IconButton onClick={() => userSt.setSettingsSecond("avatar", item.type, userSt.settings.avatar[item.type] === item.length - 1 ? 0 : userSt.settings.avatar[item.type] + 1)}>
                    <NavigateNextIcon />
                </IconButton>
            </Grid>))}
        </Grid>
                ...
                {bgcolor.map((item, index) => (
                    ...
                    >
                        <Box onClick={() => userSt.setSettingsSecond("avatar", "bgcolor", index)} sx={{ height: 64, width: 64, m: 1, cursor: "pointer" }}>
                            <CustomAvatar avatar={{ ...userSt.settings.avatar, bgcolor: index }} height="64px" width="64px" />
                        </Box>
