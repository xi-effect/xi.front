        ...
        <>
            {
                values.content.map((item, indexA) => (
                    <Draggable key={indexA.toString()} draggableId={`list-${indexA}`} index={indexA}>
                        {provided => (
                            <Grid
...
const List = inject('managmentStore')(observer(({ managmentStore, index }) => {
            ...
            >
                <DragDropContext onDragEnd={onDragEnd}>
                    <Droppable droppableId={`list-${index}`}>
                        {provided => (
                            <Grid sx={{ width: "calc(100% - 4px)", }} ref={provided.innerRef} {...provided.droppableProps} >
