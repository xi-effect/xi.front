...
const filters = ['all', 'live', 'school', 'homework', 'club'];
...
const Task = inject()(
  observer(({ task, nextTask }) => {
    ...
    const getBreak = () => {
      ...
      return `${Math.floor(diff / 60)} час${getHourEndStr(Math.floor(diff / 60))} ${
        diff % 60
      } минут`;
    ...
    return (
      <>
        ...
        >
          {moment().isBetween(dateStart, dateEnd) && (
            <ArrowForwardIosIcon
              sx={{
          ...
          <Paper
            ...
            sx={{
              ...
              height:
                60 * (dateEnd.get('hour') - dateStart.get('hour')) +
                (dateEnd.get('minute') - dateStart.get('minute')),
              minHeight: '64px',
              width: 'calc(100% - 88px)',
              // minWidth: 300,
        ...
        {nextTask != null && (
          <Divider sx={{ color: 'text.secondary', fontSize: 14, mt: 2 }}>
            {' '}
            {`перерыв ${getBreak()}`}
          </Divider>
        )}
        ...
        >
          {tasks.map((task, index) => (
            <Task
              key={index.toString()}
              task={task}
              nextTask={index + 1 <= tasks.length ? tasks[index + 1] : null}
            />
          ))}
        </Stack>
      </>
