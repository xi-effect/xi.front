...
import { EditorState, RichUtils, Modifier, SelectionState, AtomicBlockUtils } from 'draft-js';
...
export function removeCurrentBlockText(editorState: EditorState) {
  ...
  const afterRemovalContentState = Modifier.removeRange(contentState, newSelection, 'backward');
...
export function insertCustomEmoji(editorState: EditorState, src: string) {
  const contentState = editorState.getCurrentContent();
  const contentStateWithEntity = contentState.createEntity('emoji', 'IMMUTABLE', { src });
...
// export function insertUploadedFiles(editorState: EditorState, files: UploadFile[]) {
//   if (files.length < 1) return editorState;

//   const contentState = editorState.getCurrentContent();
//   const contentStateWithEntity = contentState.createEntity('upload', 'IMMUTABLE', { files });
...
export function getSearchText(editorState: EditorState, selection: SelectionState): WordAtResult {
  const anchorKey = selection.getAnchorKey();
