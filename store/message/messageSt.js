...
class MessageSt {
  ...
  @action searchUsers = () => {
    this.rootStore
      .fetchData(`${this.rootStore.url}/users/`, 'POST', {
        search: this.dialogChatCreation.search,
        counter: 0,
      })
      .then((data) => {
  ...
  @action selectUserInSearch = (index) => {
    ...
    this.dialogChatCreation.searchResults = this.dialogChatCreation.searchResults.filter(
      (n, id) => {
  ...
  @action createChat = () => {
    ...
    this.rootStore
      .fetchData(`${this.rootStore.url}/chats/`, 'POST', { name: this.dialogChatCreation.chatName })
      .then((data) => {
        this.setChat('id', data.id);
        this.rootStore
          .fetchData(`${this.rootStore.url}/chats/${this.chat.id}/users/add-all/`, 'POST', {
            ids: users,
          })
          .then(() => {
  ...
  @action loadMetaForChat = (id) => {
    this.setChat('id', id);
    this.rootStore.fetchData(`${this.rootStore.url}/chats/${id}/`, 'GET').then((data) => {
  ...
  @action loadUsersForChat = (id) => {
    this.rootStore
      .fetchData(`${this.rootStore.url}/chats/${id}/users/`, 'POST', { counter: 0 })
      .then((data) => {
  ...
  @action uploadFirstMessages = (id) => {
    ...
    this.rootStore
      .fetchData(`${this.rootStore.url}/chats/${id}/message-history/`, 'POST', { counter: 0 })
      .then((data) => {
  ...
  @action uploadMoreMessages = () => {
    // this.setChat("uploading", true)
    this.rootStore
      .fetchData(`${this.rootStore.url}/chats/${this.chat.id}/message-history/`, 'POST', {
        counter: this.chat.messageCounter,
      })
      .then((data) => {
