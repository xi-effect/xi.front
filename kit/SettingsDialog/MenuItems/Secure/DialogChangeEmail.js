...
import { Stack, Dialog, DialogTitle, DialogContentText, InputAdornment, IconButton, Button } from "@mui/material";
...
const DialogChangeEmail = inject("rootStore")(observer(({ rootStore, openEmailChangeDialog, setOpenEmailChangeDialog }) => {
    ...
    const onSubmit = (newData) => {
        ...
        rootStore.fetchData(`${rootStore.url}/email-change/`, "POST", { "password": Crypto.SHA384(newData.password).toString(), "new-email": newData.newEmail })
            .then((data) => {
                if (data !== undefined) {
                    if (data.a === "Success") {
    ...
    return (
        <Dialog open={openEmailChangeDialog} onClose={() => {
            reset();
            setOpenEmailChangeDialog(false);
            ...
            >
                ...
                >
                    <Button color="inherit" onClick={() => setOpenEmailChangeDialog(false)}>отмена</Button>
