...
const ChatDialog = inject('messageSt')(
  observer(({ messageSt }) => (
    <Dialog
    ...
    >
      <DialogTitle sx={{ bgcolor: 'background.2' }} id="scroll-dialog-title">
        <Stack direction="column" justifyContent="center" alignItems="flex-start" spacing={1}>
          ...
          {messageSt.dialogChatCreation.usersForChat.length === 0 && (
            <Typography sx={{ width: '100%' }} align="center" variant="subtitle1">
              {' '}
              Вы пока не выбрали ни одного пользователя{' '}
            </Typography>
          )}
          {messageSt.dialogChatCreation.usersForChat.length !== 0 &&
            messageSt.dialogChatCreation.usersForChat.map((item, index) => (
              <Stack
                key={index.toString()}
                direction="row"
                ...
                >
                  ...
                  <Grid container wrap="nowrap" spacing={2}>
                    <Grid item xs zeroMinWidth>
                      <Typography sx={{ maxWidth: 400 }} noWrap>
                        {item.bio}
                      </Typography>
                    </Grid>
                  </Grid>
      ...
      <DialogContent sx={{ bgcolor: 'background.2' }} dividers>
        ...
        >
          ...
          {messageSt.dialogChatCreation.searchResults.length === 0 && (
            <Typography sx={{ width: '100%' }} align="center" variant="subtitle1">
              {' '}
          ...
          {messageSt.dialogChatCreation.searchResults.length !== 0 &&
            messageSt.dialogChatCreation.searchResults.map((item, index) => (
              <Stack
                key={index.toString()}
                direction="row"
                justifyContent="flex-start"
      ...
      <DialogActions sx={{ bgcolor: 'background.2' }}>
        <Button
          sx={{ color: 'text.main' }}
          onClick={() => messageSt.setDialogChatCreation('openDialog', false)}
        >
          Отмена
        </Button>
        <Button sx={{ color: 'text.main' }} onClick={() => messageSt.createChat()}>
          Готово
        </Button>
      </DialogActions>
    </Dialog>
