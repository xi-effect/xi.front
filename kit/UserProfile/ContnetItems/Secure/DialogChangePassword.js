...
const DialogChangePassword = inject(
  'rootStore',
  'userSt',
)(
  observer(({ rootStore, openPasswordChangeDialog, setOpenPasswordChangeDialog }) => {
    ...
    const onSubmit = (newData) => {
      ...
      rootStore
        .fetchData(`${rootStore.url}/password-change/`, 'POST', {
          ...
          'new-password': Crypto.SHA384(newData.newPassword).toString(),
        })
        .then((data) => {
          if (data !== undefined) {
            if (data.a === 'Success') {
    ...
    return (
      ...
      >
        ...
        >
            ...
            <Controller
              ...
              render={({ field }) => (
                <TextFieldCustom
                  ...
                  label="Новый пароль"
                  helperText={
                    errors?.newPassword && 'Пароль должен иметь минимальную длинну в 6 символв'
                  }
                  InputProps={{
                    endAdornment: (
                      <InputAdornment position="end">
          ...
          >
            <Button color="inherit" onClick={() => setOpenPasswordChangeDialog(false)}>
              отмена
            </Button>
